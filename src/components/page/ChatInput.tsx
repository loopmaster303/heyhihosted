...
const ChatInput: React.FC<ChatInputProps> = ({
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      const newHeight = Math.min(Math.max(textareaRef.current.scrollHeight, 24), 130);
            ...
            <div className="relative flex items-center p-2">
                ...
                <div className="flex-grow mx-2">
                    <Textarea
                        ...
                        className="w-full bg-transparent text-foreground placeholder:text-muted-foreground focus-visible:ring-0 focus-visible:ring-offset-0 border-0 shadow-none p-2 m-0 leading-tight resize-none overflow-y-auto no-scrollbar"
                        rows={1}
                        disabled={isLoading || isRecording || isTranscribing}
                        aria-label="Chat message input"
                        style={{ lineHeight: '1.5rem', height: '24px' }}
                    />
                </div>
