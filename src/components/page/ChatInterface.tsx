...
const ChatInterface: React.FC = () => {
    ...
    // Only show welcome screen on hard page reload (not on "New Chat" clicks)
            ...
            <div className="flex-grow overflow-y-auto pt-[72px] pb-4 px-4 no-scrollbar">
                {shouldShowWelcome ? (
                    <WelcomeScreen
                        ...
                        activeMode={isImageMode ? 'visualize' : 'chat'}
                        onModeChange={(mode) => {
                            if (mode === 'visualize' && !isImageMode) toggleImageMode();
                            if (mode === 'chat' && isImageMode) toggleImageMode();
                        }}
                    />
                ) : (
