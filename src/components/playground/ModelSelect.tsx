...
export function ModelSelect({ entries, mode, value, onChange, loading, fallbackActive }: Props) {
  ...
  return (
    <div className={`${styles.select} ${open ? styles.open : ''}`}>
      {fallbackActive && <p className={styles.warn}>Offline list — live registry unavailable</p>}
      <button
      ...
      >
        ...
        <span className={styles.modelName}>{current?.name ?? (loading ? 'Loading…' : 'No model')}</span>
      </button>
      {open && (
