...
| `src/config/translations.ts` | Neuer Schlüssel `modelSelector.pollenRequired`; `playground.prunaEmpty` neu geschrieben; *Korrektur 2026-08-29:* die drei toten Schlüssel `playground.title`, `playground.cancel`, `playground.enhance` wurden **nicht** entfernt — die Zeile war falsch. Sie stehen weiter im Baum (Paket W6 des Audit-Patch-Plans). |
